...
protocol Operation {
    ...
    func execute(in dispatcher: D, completion: @escaping (FlightResult<R, Error>) -> Void)
}
...
extension Operation {
    ...
    func executeBaseResponse<T: Codable>(dispatcher: Dispatcher, completion: @escaping (FlightResult<T, Error>) -> Void) {
        do {
    ...
    ///   - dispatcher: The dispatcher
    ///   - completion: The completion handler
    func executeBaseArrayResponse<T: Codable>(dispatcher: Dispatcher, completion: @escaping (FlightResult<[T], Error>) -> Void) {
        do {
            try dispatcher.execute(request: self.request, completion: { (xresponse: Response) in
                if let json = xresponse.data {
                    let decoder = JSONDecoder()
